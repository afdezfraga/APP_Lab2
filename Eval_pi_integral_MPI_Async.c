...
char *argv[];
{
    ...
    ****************** */
    ...
    MPI_Request requestB, requestR1, requestR2;
    MPI_Init(&argc, &argv);
    ...
    t = MPI_Wtime();
    // Bcast N
    MPI_Ibcast( &n , 1 , MPI_INT , MPI_ROOT_PROCESS , MPI_COMM_WORLD, &requestB);
    ...
    MPI_Ireduce( &pi_local, &pi, 1, MPI_DOUBLE, MPI_SUM, MPI_ROOT_PROCESS, MPI_COMM_WORLD, &requestR1);
    MPI_Wait( &requestR1, MPI_STATUS_IGNORE);
    t = MPI_Wtime() - t ;
    MPI_Ireduce( &t, &t_max, 1, MPI_DOUBLE, MPI_MAX, MPI_ROOT_PROCESS, MPI_COMM_WORLD, &requestR2);
    MPI_Wait( &requestR2, MPI_STATUS_IGNORE);
