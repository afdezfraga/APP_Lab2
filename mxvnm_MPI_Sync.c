...
void distribute_data( int rank, int number_of_processes, 
  ...
  // Distribute vector x
  MPI_Bcast( *(x), *M, MPI_FLOAT, ROOT_PROCESS, MPI_COMM_WORLD);
  ...
  *n_local = (rank < n_resto) ? n_div+1 : n_div;
  ...
  if ((*(y_local) = (float *)malloc( (*n_local) * sizeof(float))) == NULL)
    printf("Error en malloc y[%d]\n", *(N) );
  ...
  MPI_Scatterv( Avector, sendcounts, displs, 
...
void gather_results(float *y, float *y_local, int n_local, 
  ...
  MPI_Gatherv(y_local, n_local, MPI_FLOAT, 
